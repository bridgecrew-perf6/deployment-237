...
export default function EventHistory() {
  ...
  const [errorPromoterFee, setErrorPromoterFee] = useState(initialError);
  ...
  const [promoterFeeValue, setPromoterFeeValue] = useState({ val: [] });
  ...
  const onInputPackagePromoterFee = (e, index) => {
    const { name, value } = e.target;
    const rows = [...promoterFeeValue.val];
    rows[index] = { [name]: value };
    setPromoterFeeValue({ val: rows });
  };
  ...
  const dynamicJson = (index) => {
    ...
    const promoterFee = parseInt(promoterFeeValue.val[index]["promoterFee"]);
    const handlingFee = handlingFeeValue.val[index]["handlingFee"];
    const sellingPrice = Math.round((amount * (100 - promoterFee)) / 100);
    return {
      package_name:
      ...
      promoterFee:
        promoterFeeValue.val[index] != null
          ? promoterFeeValue.val[index]["promoterFee"]
          : "",
      selling_price:
        promoterFeeValue.val[index]["promoterFee"] != ""
          ? sellingPrice
  ...
  const handleEdit = (id) => {
    ...
    if (pack.length > 0) {
      ...
      const rows2 = [...promoterFeeValue.val];
      ...
      for (let i = 0; i <= pack.length - 1; i++) {
        ...
        rows2[i] = {
          ["promoterFee"]: pack[i].promoterFee,
        };
        rows4[i] = {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      requestObj.append("promoterFee", filteredData.promoterFee);
      ...
      requestObj.append("cccharge", filteredData.cccharge);
      ...
      const response = await API.update_event(requestObj);
  ...
  const handleEventDelete = async () => {
    try {
      const response = await API.delete_event(blogId);
      if (response.status == 200) {
        setIsDelete(false);
        setIsEdit(false);
      }
  ...
  const handleEventStatus = async (e, item) => {
    let tempData = [...eventData];
    let fId = tempData.findIndex(fData => fData.id === item.id);
    tempData[fId].is_active = e.target.checked ? 1 : 0;
    setEventData(tempData);
    let reqObj = {
      id: item.id,
      is_active: e.target.checked ? 1 : 0
    }
    await API.update_event_status(reqObj).then(res => {
      EventHistory();
    }).catch(err => {
      console.log(err);
    })
  ...
  return (
    <div className="w-full h-auto rounded-xl shadow-lg bg-white p-4">
      <div className="uppercase font-medium text-secondary">Event</div>
      <div className=" font-medium text-gray-400 mb-2">All Event History</div>
      <hr />
      {isEmpty ? (
        <div>Events Empty</div>
      ) : (
        <TableContainer>
              ...
              {eventData.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="w-32">{date}</TableCell>
                    <TableCell>{item.title}</TableCell>
                    <TableCell className="capitalize w-36">
                      {item.category}
                    </TableCell>
                      ...
                      <FormControlLabel
                        control={
                          <IOSSwitch
                            ...
                            onChange={(e) => handleEventStatus(e, item)}
                            name="checkedB"
                          />
                        }
                        className="pl-2"
      ...
      {isEdit && (
        ...
        >
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left  shadow-xl transform transition-all sm:my-8 sm:align-middle w-full lg:w-1/2 xl:w-1/2">
                    ...
                    <div className="w-1/4 px-3 mb-3">
                      ...
                      <div className="flex">
                        ...
                        >
                          ...
                          <option
                            value="0"
                            selected={filteredData.exclusive == "0" && true}
                          >
                            No
                          </option>
                        </select>
                  ...
                  <div className="xl:w-1/3 lg:w-1/3 w-full px-3 mb-3">
                    ...
                    <div className="flex">
                      ...
                      <input
                        ...
                        name="cccharge"
                        id="tax"
                        value={filteredData.cccharge}
                        onChange={handleEventEdit}
                        className="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-innerBG"
                        placeholder="Creditcard Charges in %"
                      />
                    </div>
                    {errorAddress.field === "cccharge" && (
                      <p className="text-xs text-red-600 mt-2">
                        {errorAddress.message}
                      </p>
                    )}
                              ...
                              <div>
                                ...
                                <div className="flex">
                                  ...
                                  <Number
                                    ...
                                    name="promoterFee"
                                    value={
                                      promoterFeeValue.val[index]
                                        ? promoterFeeValue.val[index].promoterFee
                                        : ""
                                    }
                                    id={`promoterFee${index}`}
                                    onChange={(e) => {
                                      onInputPackagePromoterFee(e, index);
                                    }}
                                  />
                                </div>
                                {errorPromoterFee.field === "promoterFee" && (
                                  <p className="text-xs text-red-600 mt-2 absolute">
                                    {errorPromoterFee.message}
                                  </p>
                                )}
                              </div>
                            </div>
