...
export default function EventHistory() {
  ...
  const dynamicJson = (index) => {
    const amount = parseInt((amountValue.val && amountValue.val[index] && amountValue.val[index]["amount"]) ? amountValue.val[index]["amount"] : 0);
    const discount = parseInt((discountValue.val && discountValue.val[index] && discountValue.val[index]["discount"]) ? discountValue.val[index]["discount"] : 0);
    const sellingPrice = Math.round((amount * (100 - discount)) / 100);
    const tax = parseInt((taxValue.val && taxValue.val[index] && taxValue.val[index]["tax"]) ? taxValue.val[index]["tax"] : 0);
    const handlingFee = parseInt((handlingFeeValue.val && handlingFeeValue.val[index] && handlingFeeValue.val[index]["handlingFee"]) ? handlingFeeValue.val[index]["handlingFee"] : 0);
    return {
      package_name:
        values.val[index] != null ? values.val[index]["package_name"] : "",
      amount:
        amountValue.val[index] != null ? amountValue.val[index]["amount"] : "",
      discount: discountValue.val[index] != null ? discountValue.val[index]["discount"] : "",
  ...
  useEffect(() => {
    ...
    async function EventHistory() {
      try {
        ...
        if (response.data[0].events.length) {
          setIsEmpty(false);
        }
        setEventData(data);
      } catch (e) { }
  ...
  const setEventTags = (tags) => {
    let tempTags = [...selTags];
    tags.map(tData => {
      tempTags.push({ label: tData, value: tData });
    });
    setSelTags(tempTags);
  ...
  const handleEdit = (id) => {
    ...
    if (pack.length > 0) {
      ...
      for (let i = 0; i <= pack.length - 1; i++) {
        ...
        rows3[i] = {
          ["tax"]: pack[i].tax ? pack[i].tax : 0,
        };
        rows4[i] = {
          ["handlingFee"]: pack[i].handlingFee ? pack[i].handlingFee : 0,
        };
      }
      setValues({ val: rows });
  ...
  const handleEditor = (e, fieldName) => {
    // setFilteredData({ ...filteredData, [e.editor.name]: e.editor.getData() });
    ...
    <div className="w-full h-auto rounded-xl shadow-lg bg-white p-4">
      ...
      {isEdit && (
       ...
       py-2 px-2 h-10  text-sm tracking-normal rounded-full border border-white hover:to-primary hover:from-secondary uppercase text-white transition duration-500 ease-in-out focus:outline-none"
                  ...
                  {/* Tags */}
                  <div className=" w-full px-3 mt-3 mb-7 relative">
                    <label
                      htmlFor="secret_key"
                      className="text-xs font-semibold px-1 text-theme"
                    >
       ...
       py-2 lg:px-4 xl:px-4 px-8 lg:w-44 xl:w-44 w-auto  text-sm tracking-normal rounded-3xl border border-white hover:to-primary hover:from-secondary uppercase text-white transition duration-500 ease-in-out focus:outline-none mx-2 opacity-70"
                  onClick={() => { setSelTags([]); setIsEdit(false); }}
                >
                  Cancel
                </button>
              </div>
