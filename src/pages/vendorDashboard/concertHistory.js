...
export default function ConcertHistory() {
  ...
  const dynamicJson = (index) => {
    ...
    let tax , handlingFee = 0;
    if(taxValue.val && taxValue.val['index'] && taxValue.val[index]["tax"])
    {
      tax = parseInt(taxValue.val[index]["tax"]);
    } else {
      tax = 0;
    }
    if(handlingFeeValue.val && handlingFeeValue.val['index'] && handlingFeeValue.val[index]["handlingFee"])
    {
      handlingFee = parseInt(handlingFeeValue.val[index]["handlingFee"]);
    } else {
      handlingFee = 0;
    }
    return {
  ...
  useEffect(() => {
    async function EventHistory() {
      try {
        const response = await API.concert_by_vendor(vendor.id);
        const data = [];
        response.data[0].concert.map((item) => {
          data.push(item);
        });
        if (data) {
          setEventData(data);
